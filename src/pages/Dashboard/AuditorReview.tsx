...
const buildAuditReport = (
    ...
    const evidenceLines = draft.evidenceFiles.map((file) => `- ${file.name} (${file.type || 'arquivo'}) -> ${file.localUri}`);
...
export default function AuditorReview() {
    ...
    const addLocalEvidenceFiles = (project: AuditItem, files: FileList | null) => {
        ...
        const evidenceFiles = acceptedFiles.map((file, index) => ({
            ...
            localUri: `local://auditoria/${projectKey}/${encodeURIComponent(file.name)}`,
        }));
        setEvidenceError(
    ...
    const removeLocalEvidenceFile = (fileId: string) => {
        setDraft((current) => ({
            ...current,
    ...
    const verify = async (project: AuditItem, status: AuditDecision) => {
        ...
        const evidenceUrls = [
            ...
            ...draft.evidenceFiles.map((file) => file.localUri),
        ];
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 px-4 py-5 text-center transition hover:border-blue-300 hover:bg-blue-50">
                                                ...
                                                <span className="mt-1 text-xs text-gray-500">Evidência local até definirmos upload real</span>
                                                <input
                                                    type="file"
                                                    multiple
                                                    ...
                                                    className="sr-only"
                                                    onChange={(event) => {
                                                        addLocalEvidenceFiles(project, event.currentTarget.files);
                                                    ...
                                                    {draft.evidenceFiles.map((file) => (
                                                        <div key={file.id} className="flex items-center justify-between gap-3 rounded-xl border border-gray-100 bg-white px-3 py-2">
                                                            <div className="min-w-0">
                                                                ...
                                                                <p className="break-all text-xs text-gray-500">{file.localUri}</p>
                                                            </div>
                                                            <button
                                                                type="button"
                                                                onClick={() => removeLocalEvidenceFile(file.id)}
