...
- [ ] **CERT-01**: A certificadora abre revisão detalhada com baseline, documentos, QTAGs e cálculo de potencial.
- [ ] **CERT-02**: Aprovação/reprovação permite notas, metodologia, potencial de crédito e motivo estruturado.
- [ ] **CERT-03**: Certificado digital ou referência documental é registrado e exibido no projeto.
- [ ] **CERT-04**: Aprovação aciona ou prepara explicitamente o fluxo de lastro/mint bloqueado com status visível.
- [ ] **CERT-05**: Histórico de decisões por projeto fica disponível para certificadora e dossiê público quando aplicável.
...
- [ ] **INTG-01**: Toda submissão relevante (propriedade, metodologia, direito de operar) gera um registro `Claim` com `status` e `confidenceScore` próprios, sem promover `projects.status` diretamente a partir do input do usuário.
- [ ] **INTG-02**: `Evidence` referencia hash, origem, `validationMethod` e `validationStatus`, vinculada a um `Claim`.
- [ ] **INTG-03**: `Conflict` é gerado automaticamente quando `ST_Intersects` (Phase 4.1) encontra overlap acima do limiar configurável, com severidade `CLEAR → CRITICAL`.
- [ ] **INTG-04**: Risk Score é calculado com sinais explicáveis (ex.: "+30 SIGEF diverge da geometria") e projetos `CRITICAL` entram automaticamente em `ON_HOLD`.
