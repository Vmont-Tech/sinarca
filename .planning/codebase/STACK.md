...
- **Python 3.11:** backend FastAPI atual em `backend/`, com manifesto em `pyproject.toml`.
...
- `backend/main.py` usa env vars para sessão, upload, taxas e fallback estático.
- `backend/services/stellar_service.py` usa env vars `STELLAR_*`.
- `backend/core/database.py` espera `DATABASE_URL`, mas não é chamado pela API ativa.
- Supabase ainda não aparece no código ativo.
