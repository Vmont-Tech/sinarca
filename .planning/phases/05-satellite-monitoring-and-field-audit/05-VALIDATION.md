...
| SATM-07 | SATM-07 | Nenhum campo de baseline exibido vem de `deterministic_baseline()` quando `CopernicusProvider` está configurado | integration | `uv run pytest -q tests/modules/satellite/test_historical_reconstruction.py -k baseline -x && uv run pytest -q tests/contract/test_backend_app_api_v1.py -k "satellite or dossier" -x` | ✅ (Plans 05/07 criam/estendem) | ⬜ pending |
